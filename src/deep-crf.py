...
MODEL_NAME = "lstm-crf_model.hd5"
RESULT_FILE = "LSTM-CRF_RESULTS.txt"
ENCODING = 'utf-16'
...
def train_model(no_of_epochs, tt_factor, tv_factor, initial, data_file):
    ...
    if initial:
        model = Sequential()
        model.add(Embedding(VOCABULARY_SIZE, EMBEDDING_DIMENSION, mask_zero=True))  # Random embedding
        model.add(Bidirectional(LSTM(BiRNN_UNITS // 2, return_sequences=True, kernel_initializer='RandomNormal')))
        model.add(Dropout(0.20))
        crf = CRF(MAX_SENTENCE_LEN, sparse_target=True, batch_size=1)
        model.add(crf)
